...
public class AgentLoader {
	 ...
	 * Internal class.
	 */
	// used internally, has to be public to be accessible from the
	// shadeClassLoaders
	...
	/**
	 * Dynamically loads a java agent. Deals with the problem of finding the
	 * proper jvm classes.
	 *
	 * @param agentJar
	 *            the agent jar
	...
	/**
	 * Creates loads the agent class directly. The agent class must be visible
	 * from the system class loader.
	 * <p/>
	 * This method creates a temporary jar with the proper manifest and loads
	 * the agent using the jvm attach facilities.
	 * <p/>
	 * This will not work if the agent class can't be loaded by the system class
	 * loader. <br>
	 * This can be worked around like by adding the specific class and any other
	 ...
	 *            if the ability to redefine classes is need by the agent,
	 *            suggested default: false
	 * @param canRetransformClasses
	 *            if the ability to retransform classes is need by the agent,
	 *            suggested default: false
	 * @param canSetNativeMethodPrefix
	 *            if the ability to set native method prefix is need by the
	 *            agent, suggested default: false
	 * @see ClassPathUtils
	 * @see java.lang.instrument.Instrumentation
	 */
	public static void loadAgentClass(final String agentClass, final String options, final String bootClassPath,
	...
	private synchronized static AgentLoaderInterface getAgentLoader(final String agentJar) {
		...
		try {
			...
			final List<String> shaded = Arrays.asList("shaded/AttachProvider.class", "shaded/VirtualMachine.class",
					"AttachProviderPlaceHolder.class", "shaded/AgentInitializationException.class",
					"shaded/AgentLoadException.class", "shaded/AttachNotSupportedException.class",
					"shaded/AttachOperationFailedException.class", "shaded/AttachPermission.class",
					"shaded/HotSpotAttachProvider.class", "shaded/HotSpotVirtualMachine.class",
			...
			for (String s : shaded) {
				...
				// to prevent getting "Native Library .../libattach.so already
			...
			try {
				agentLoaderClass = (Class<AgentLoaderInterface>) ClassPathUtils.defineClass(systemLoader,
		...
		try {
			...
			// this reflection based implementation of this interface solves
			// this problem.
			agentLoader = new AgentLoaderInterface() {
				
				public void loadAgent(final String agentJar, final String options) {
					try {
						final Method loadAgentMethod = agentLoaderObject.getClass().getMethod("loadAgent", String.class,
								String.class);
	...
	/**
	 * Creates a jar in runtime with the proper manifest file to start the
	 * javaagent.
	 * <p/>
	 * This method is convenient to java agent developers since they can test
	 * their agents without creating a jar first.
	 *
	 * @param agentClass
	 *            the agent class
	 * @param bootClassPath
